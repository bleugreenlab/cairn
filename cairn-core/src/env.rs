...
pub(crate) const CAIRN_SHIM_MARKER: &str = "cairn-managed-shim";
...
/// the CAIRN-2422 incident), and exec the real jj for everything else —
/// honoring `$CAIRN_JJ_BIN` first (operator/test override), else the absolute
/// bundled path baked in at install time.
///
/// The interception ALWAYS narrates itself on stderr and names the escape hatch,
/// even though it still exits 0 so the `jj workspace update-stale && jj st`
/// idiom proceeds. This shim previously exited 0 in silence on the argument that
/// a no-op has nothing to report. That argument is wrong, and CAIRN-3223 priced
/// it: `jj` resolves through `<cairn_home>/bin` for agent shells AND for the
/// operator's interactive shell, so a real stale-store incident was diagnosed
/// against a command that exited clean and changed nothing — for an evening,
/// from two different shells, while real jj (verified on scratch stores against
/// jj 0.42) would have repaired it on the first try. A silent no-op is
/// indistinguishable from a successful repair, which is the one thing a
/// diagnostic surface may never be.
// Available in every test build, not just Unix ones: the shim-unwrapping guard
// below must understand BOTH generated formats on whichever platform runs the
// suite, mirroring `jj_shim_script_windows`.
#[cfg(any(unix, test))]
fn jj_shim_script_unix(jj_abs: &str) -> String {
    format!(
        r#"#!/bin/sh
# {CAIRN_SHIM_MARKER} (jj) — generated by env.rs. Cairn keeps workspaces current
# itself, serialized against concurrent store operations; hand-running
# jj workspace update-stale can race a rebase and drop sealed commits, so this
# shim intercepts exactly that one command and execs the real jj for everything
# else. The interception always says so: a silent clean exit reads as a repair.
if [ "$1" = "workspace" ] && [ "$2" = "update-stale" ]; then
  echo "cairn: intercepted 'jj workspace update-stale' and changed nothing. Cairn refreshes workspaces itself, serialized against store operations; a hand-run refresh can race a rebase and drop sealed commits. If you are diagnosing a stale workspace, run the real jj directly: \"${{CAIRN_JJ_BIN:-{jj_abs}}}\" workspace update-stale" 1>&2
  exit 0
fi
...
/// The Windows `jj.cmd` shim body: same narrated interception + forwarding as
/// the Unix script, structured with `goto` labels rather than parenthesized
/// blocks.
#[cfg(any(windows, test))]
fn jj_shim_script_windows(jj_abs: &str) -> String {
    format!(
        "@echo off\r\nrem {CAIRN_SHIM_MARKER} (jj)\r\nif \"%~1\"==\"workspace\" if \"%~2\"==\"update-stale\" goto __cairn_stale\r\nif defined CAIRN_JJ_BIN goto __cairn_envbin\r\n\"{jj_abs}\" %*\r\nexit /b %ERRORLEVEL%\r\n:__cairn_envbin\r\n\"%CAIRN_JJ_BIN%\" %*\r\nexit /b %ERRORLEVEL%\r\n:__cairn_stale\r\necho cairn: intercepted 'jj workspace update-stale' and changed nothing. Cairn refreshes workspaces itself; to diagnose a stale workspace run the real jj at \"{jj_abs}\" directly. 1>&2\r\nexit /b 0\r\n"
    )
}
...
/// The real jj a Cairn-generated `jj` shim forwards to, or `None` when `bin` is
/// not one of our shims (a real binary is not UTF-8 and falls out here).
///
/// The crate's real-jj suites resolve their binary through this, because
/// `<cairn_home>/bin` leads PATH in agent shells and, once Cairn is installed,
/// in the developer's. A suite that resolved the shim would drive an
/// unconditional `jj workspace update-stale` no-op while believing it was
/// driving jj. Both generated formats are understood: the shim on PATH is
/// whichever one the host platform installed.
#[cfg(test)]
pub(crate) fn real_jj_behind_shim(bin: &str) -> Option<String> {
    let body = std::fs::read_to_string(resolve_executable(bin)?).ok()?;
    cairn_jj_shim_target(&body)
}

/// The forwarding target baked into a generated jj shim body.
///
/// Deliberately the BAKED absolute path rather than the `CAIRN_JJ_BIN` override
/// both formats prefer at runtime: the caller consulted that override before it
/// ever looked at a shim, so following it here would resolve a shim whose
/// override points at itself straight back to itself.
#[cfg(test)]
fn cairn_jj_shim_target(body: &str) -> Option<String> {
    if !body.contains(CAIRN_SHIM_MARKER) {
        return None;
    }
    // Unix: `exec "${CAIRN_JJ_BIN:-<abs>}" "$@"`.
    if let Some((_, rest)) = body.split_once("exec \"${CAIRN_JJ_BIN:-") {
    ...
    // Windows: `"<abs>" %*`, beside a `"%CAIRN_JJ_BIN%" %*` branch that is an
    // expansion rather than a path.
    body.lines()
        .filter_map(|line| line.trim().strip_suffix(" %*"))
        .filter_map(|quoted| quoted.strip_prefix('"')?.strip_suffix('"'))
        .find(|target| !target.is_empty() && !target.contains('%'))
        .map(ToOwned::to_owned)
}

/// Resolve a command name the way the host platform's launcher would: an
/// explicit path is taken as given, and a bare name searches PATH. On Windows
/// each candidate is probed with the PATHEXT extensions, which is how a bare
/// `jj` finds Cairn's generated `jj.cmd`.
#[cfg(test)]
fn resolve_executable(bin: &str) -> Option<PathBuf> {
    if Path::new(bin).is_absolute() || bin.contains('/') || bin.contains('\\') {
        return executable_candidates(PathBuf::from(bin), cfg!(windows)).find(|c| c.is_file());
    }
    std::env::split_paths(&std::env::var_os("PATH")?)
        .find_map(|dir| executable_candidates(dir.join(bin), cfg!(windows)).find(|c| c.is_file()))
}

/// `base` itself, plus — when `windows`, and when `base` carries no extension —
/// `base` with each executable suffix. `.CMD` is always probed regardless of
/// what PATHEXT holds, because it is the extension Cairn itself generates and
/// therefore the one this lookup exists to find.
///
/// `windows` is a parameter rather than a `cfg!` so the probing has real
/// coverage on every platform that runs the suite, instead of only wherever a
/// Windows job happens to exist.
#[cfg(test)]
fn executable_candidates(base: PathBuf, windows: bool) -> std::vec::IntoIter<PathBuf> {
    let mut candidates = vec![base.clone()];
    if windows && base.extension().is_none() {
        let configured =
            std::env::var("PATHEXT").unwrap_or_else(|_| ".COM;.EXE;.BAT;.CMD".to_string());
        let mut suffixes: Vec<String> = configured
            .split(';')
            .map(str::trim)
            .filter(|ext| !ext.is_empty())
            .map(str::to_owned)
            .collect();
        if !suffixes.iter().any(|ext| ext.eq_ignore_ascii_case(".cmd")) {
            suffixes.push(".CMD".to_string());
        }
        for ext in suffixes {
...
mod tests {
    ...
    #[test]
    fn jj_shim_script_unix_narrates_its_interception_and_forwards() {
        let script = jj_shim_script_unix("/opt/cairn/jj");
        assert!(script.starts_with("#!/bin/sh\n"));
        assert!(script.contains(CAIRN_SHIM_MARKER));
        // Intercepts exactly `workspace update-stale` and exits 0, so the common
        // `jj workspace update-stale && jj st` idiom still reaches `jj st`.
        assert!(script.contains(r#"if [ "$1" = "workspace" ] && [ "$2" = "update-stale" ]; then"#));
        assert!(script.contains("exit 0"));
        // The interception must NEVER be silent: an unexplained clean exit is
        // indistinguishable from a successful repair, which is what made
        // CAIRN-3223's stale store cost an evening to diagnose.
        assert!(script.contains("1>&2"));
        assert!(script.contains("changed nothing"));
        // And it must name the escape hatch — the real jj, by resolvable path.
        assert!(script.contains(r#"\"${CAIRN_JJ_BIN:-/opt/cairn/jj}\" workspace update-stale"#));
        // Backticks inside the double-quoted message would be command
        // substitution under /bin/sh; the message quotes with apostrophes.
    ...
    /// The shim-unwrapping guard must understand BOTH generated formats, on
    /// whichever platform runs the suite. It previously read only the Unix
    /// script, so a Windows developer with Cairn installed kept `jj.cmd` as the
    /// binary under test: ordinary commands forwarded, but `update_stale` was
    /// intercepted, and the real-jj staleness suite tested the no-op.
    #[test]
    fn shim_forwarding_target_is_read_from_both_generated_formats() {
        assert_eq!(
            cairn_jj_shim_target(&jj_shim_script_unix("/opt/cairn/jj")).as_deref(),
            Some("/opt/cairn/jj")
        );
        assert_eq!(
            cairn_jj_shim_target(&jj_shim_script_windows("C:\\cairn\\jj.exe")).as_deref(),
            Some("C:\\cairn\\jj.exe"),
            "the baked absolute path, never the `%CAIRN_JJ_BIN%` forwarding branch"
        );
        // Anything that is not one of ours is left alone — including a
        // hand-written forwarder that happens to exec jj.
        assert_eq!(
            cairn_jj_shim_target("#!/bin/sh\nexec /usr/bin/jj \"$@\"\n"),
            None
        );
        assert_eq!(cairn_jj_shim_target(""), None);
    }

    /// The installed shim resolves back to its target, and a real binary is not
    /// mistaken for one. Exercises whichever format this platform installs.
    #[test]
    fn real_jj_behind_shim_unwraps_the_installed_shim() {
        let dir = tempfile::tempdir().unwrap();
        let bin_dir = dir.path().join("bin");
        let target = dir.path().join("jj-real");
        std::fs::write(&target, b"#!/bin/sh\n").unwrap();
        let target_str = target.to_string_lossy().to_string();

        ensure_jj_shim_in(&bin_dir, &target_str);
        let shim = bin_dir.join(if cfg!(windows) { "jj.cmd" } else { "jj" });
        assert!(shim.is_file(), "the platform's shim was installed");
        assert_eq!(
            real_jj_behind_shim(&shim.to_string_lossy()).as_deref(),
            Some(target_str.as_str())
        );
        assert_eq!(
            real_jj_behind_shim(&target_str),
            None,
            "a non-shim file is not unwrapped"
        );
    }

    /// A bare command name must probe the Windows executable extensions, or a
    /// bare `jj` never finds the `jj.cmd` Cairn generates there. Both modes are
    /// exercised on every platform.
    #[test]
    fn executable_candidates_probe_the_windows_extensions() {
        let base = PathBuf::from("somedir").join("jj");

        let windows: Vec<String> = executable_candidates(base.clone(), true)
            .map(|p| p.to_string_lossy().into_owned())
            .collect();
        assert_eq!(
            windows[0],
            base.to_string_lossy(),
            "the bare name is tried first"
        );
        assert!(
            windows
                .iter()
                .any(|n| n.to_ascii_lowercase().ends_with(".cmd")),
            "`.cmd` is always probed, whatever PATHEXT holds: {windows:?}"
        );

        let posix: Vec<PathBuf> = executable_candidates(base, false).collect();
        assert_eq!(
            posix.len(),
            1,
            "no extension probing off Windows: {posix:?}"
        );

        // An explicit extension is never re-suffixed.
        let explicit = PathBuf::from("somedir").join("jj.cmd");
        assert_eq!(executable_candidates(explicit, true).count(), 1);
    }

    #[test]
    fn jj_shim_script_windows_narrates_its_interception_and_forwards() {
        let script = jj_shim_script_windows("C:\\cairn\\jj.exe");
        assert!(script.contains(CAIRN_SHIM_MARKER));
        assert!(script.contains("if \"%~1\"==\"workspace\" if \"%~2\"==\"update-stale\" goto"));
        // Narrated interception, same contract as the Unix script.
        assert!(script.contains("1>&2"));
        assert!(script.contains("changed nothing"));
