...
///
/// A child linked through parent_thread_id branches from the project base.
///
...
pub(crate) async fn resolve_parent_branch(
    ...
    let mut parent_rows = conn
        .query(
            "
            SELECT child.parent_issue_id, child.parent_job_id, child.parent_thread_id
            FROM issues child
            WHERE child.id = ?1
            LIMIT 1
            ",
    ...
    if parent_row.opt_text(2)?.is_some() {
        return Ok(None);
    }
...
mod parent_tests {
    ...
    #[tokio::test]
    async fn resolve_parent_branch_returns_none_for_parent_thread_id() {
        let db = migrated_db().await;
        db.execute_script(
            "INSERT INTO workspaces(id,name,created_at,updated_at) VALUES('w','W',1,1);
