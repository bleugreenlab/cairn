...
use crate::mcp::file_targets::path_within_any;
use crate::mcp::handlers::search::{build_glob_matcher, format_grep_line};
...
impl WorktreeSearch {
    ...
    fn new(worktree: &Path) -> Result<Self, String> {
...
mod tests {
    ...
    fn grep_payload(
        ...
        glob: Option<&str>,
        output_mode: &str,
    ) -> crate::mcp::handlers::search::GrepPayload {
        crate::mcp::handlers::search::GrepPayload {
    ...
    fn cold_grep(
        ...
        crate::mcp::handlers::search::grep_search(payload, root, output_mode, true, deny_read)
            .unwrap()
    }
    ...
    fn index_glob_subdir_matches_walk_and_rebases_paths() {
        ...
        assert_eq!(
            slash_indexed,
            std::collections::HashSet::from(["nested/deep.rs".to_string()])
        );
        ...
        assert_eq!(
            fenced,
            std::collections::HashSet::from(["visible.rs".to_string()])
        );
    }
    ...
    fn glob_paths_from_walk(root: &Path, pattern: &str, deny_read: Vec<PathBuf>) -> Vec<PathBuf> {
        crate::mcp::handlers::search::glob_matched_paths_walk(
            root.to_path_buf(),
            pattern.to_string(),
            deny_read,
        )
    ...
    fn path_set(paths: Vec<PathBuf>) -> std::collections::HashSet<String> {
        paths
            .into_iter()
