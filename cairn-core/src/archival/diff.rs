...
pub fn render_commit_diff(store: &ObjectStore, commit_oid: &oid) -> Result<String, String> {
...
mod tests {
    use super::*;
    use crate::archival::objects::ObjectStore;
    use crate::archival::testutil::{commit_all, git, init_repo, write_file};
    use gix_hash::ObjectId;
    use std::path::Path;

    fn oid(hex: &str) -> ObjectId {
        ObjectId::from_hex(hex.as_bytes()).expect("valid sha")
    }
    ...
    fn render_range_diff_matches_git_diff() {
        ...
        let rendered = render_range_diff(&store, &oid(&base), &oid(&tip)).unwrap();
    ...
    fn render_range_file_diffs_reports_status_and_counts() {
        ...
        let store = ObjectStore::new(Path::new(dir), None).unwrap();
        let files = render_range_file_diffs(&store, &oid(&base), &oid(&tip)).unwrap();
