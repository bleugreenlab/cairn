...
pub async fn archive_target(
    ...
    let Classified {
        ...
        history,
        blobs,
    } = classify(worktree_path, repo_path, &loaded)?;
...
fn classify(worktree_path: &str, repo_path: &str, loaded: &Loaded) -> Result<Classified, String> {
    ...
    if eligible {
        ...
        let built = build_execution_pack(worktree, pack_anchor, &loaded.default_branch)?;
        let (pack_pair, tip_sha) = match built {
            Some((pack, idx, tip)) => (Some((pack, idx)), tip),
            None => (
                None,
                git_head(worktree)
                    .ok_or_else(|| "resolving worktree HEAD for archival".to_string())?,
            ),
        };
        store = Some(
        ...
        current = loaded.base_commit.clone();
    ...
    let write_commits = if eligible {
        build_write_commits(worktree, &loaded.events, &tool_map, &mut sha_cache)
    } else {
        HashMap::new()
    };
            ...
            match tool_name {
                ...
                Some("run") => {
                    ...
                    if eligible {
                        ...
                        {
                            if let Some(full) = resolve_full(worktree, &short, &mut sha_cache) {
                                current = Some(full);
                            }
                        }
                    }
...
fn try_read(
    ...
    match (paths, current, stored) {
        (Some(paths), Some(commit), Some(stored)) => {
            ...
            if candidate == stored {
                ...
                try_hybrid_read(event, &paths, commit, store, &stored, updates, summary)?;
            } else {
...
fn build_write_commits(
    ...
    for event in events {
        ...
        if let Some(full) = tool_result_text(&event.data)
            .and_then(|tr| change_commit_sha(&tr))
            .and_then(|short| resolve_full(worktree, &short, sha_cache))
        {
            commits.insert(id, full);
        }
    }
    commits
}
...
fn try_assistant_write(
    ...
    write_commits: &HashMap<String, String>,
    updates: &mut Vec<EventUpdate>,
    summary: &mut ArchiveSummary,
) -> Result<(), String> {
...
fn committed_write_for_event(
    ...
    for tool in tool_uses {
        ...
        let Some(commit) = write_commits.get(id) else {
            continue;
        };
        match &found {
            None => found = Some(commit.clone()),
...
async fn apply(
    ...
    db.write(move |conn| {
        ...
        Box::pin(async move {
            ...
            for EventUpdate { id, shape } in &updates {
                // The shape encodes to its exact column values; one uniform UPDATE
                // binds all six so the writer can never set a combination the
                // reader's `decode` would reject.
...
mod tests {
    ...
    async fn system_init_round_trips_and_dedup() {
        ...
        let summary = archive_target(&db, &repo, &repo, &["job".to_string()])
            .await
            .unwrap();
        ...
        let again = archive_target(&db, &repo, &repo, &["job".to_string()])
            .await
            .unwrap();
    ...
    async fn system_init_missing_skeleton_degrades_to_stub() {
        ...
        archive_target(&db, &repo, &repo, &["job".to_string()])
            .await
            .unwrap();
        assert_eq!(blob_count(&db).await, 2);
