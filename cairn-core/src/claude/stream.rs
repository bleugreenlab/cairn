            ...
            for block in blocks {
                match block {
                    ...
                    ContentBlock::ToolUse { id, name, input } => {
                        log::info!("[DEBUG-TOOLUSE] Extracted tool_use from assistant content: id={} name={}", id, name);
...
pub fn parse_event(line: &str) -> Result<(ClaudeEvent, Value), String> {
    ...
    let event: ClaudeEvent =
        serde_json::from_value(raw.clone()).map_err(|e| format!("Failed to parse event: {}", e))?;
