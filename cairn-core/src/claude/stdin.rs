...
use crate::image_cache::ImageBase64Cache;
...
///
/// If a cache is provided, it will be checked for pre-computed base64 data
/// before falling back to reading from disk.
pub fn build_message_content(
    text: &str,
    working_dir: Option<&str>,
    cache: Option<&ImageBase64Cache>,
) -> Value {
    ...
    for path_str in &image_paths {
        ...
        if path.exists() {
            // Try cache first (instant), then fall back to disk read
            let base64_data = cache.and_then(|c| c.take(&path)).or_else(|| {
...
pub fn send_user_message_with_images(
    ...
    // Always try to embed images - build_message_content handles absolute paths
    // without needing working_dir
    let message_content = build_message_content(content, working_dir, cache);
