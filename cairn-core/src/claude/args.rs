...
pub struct ClaudeArgsConfig {
    ...
    pub session_id: Option<String>, // For new sessions: pre-generated UUID
    pub resume_session_id: Option<String>, // For resuming: existing session ID
    pub prompt: String,
...
pub fn build_claude_args(config: &ClaudeArgsConfig) -> Vec<String> {
    ...
    // Add --session-id for new sessions (pre-generated UUID)
    if let Some(ref session_id) = config.session_id {
        args.push("--session-id".to_string());
        args.push(session_id.clone());
    }

    // Add --resume if resuming an existing session
