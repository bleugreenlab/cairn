...
use super::config::write_codex_config;
use super::models::discover_codex_models;
use super::protocol::CodexStdin;
use super::thread_params::{
    build_base_instructions, build_developer_instructions, build_resume_fallback_prompt,
    build_thread_fork_params, build_thread_resume_params, build_thread_start_params,
    codex_sandbox_mode, is_missing_rollout_error,
};
...
use crate::orchestrator::session::{
    assemble_prompt_segments, insert_error_event, persist_system_prompt_event,
};
...
impl AgentBackend for CodexBackend {
    ...
    fn start_session(&self, config: SessionConfig, orch: &Orchestrator) -> Result<(), String> {
        ...
        let base_instructions = build_base_instructions(workspace_instructions.as_deref());
        let developer_instructions =
            build_developer_instructions(config.system_prompt_content.as_deref());
        // Persist the effective system prompt as labeled segments; their
        // concatenation equals `build_effective_system_prompt`'s output.
        let prompt_segments = assemble_prompt_segments(
            Some(crate::system_prompt::CODEX_SYSTEM_PROMPT),
