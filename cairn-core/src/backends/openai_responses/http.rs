...
/// Refuse a stream that ended without the response reporting an outcome.
///
...
/// response completed. Treating it as success would store truncated assistant
/// text as a finished turn and — worse — hand the turn loop a function call that
/// merely happened to parse, with no status to judge it by, which is exactly the
/// signal the loop's truncation guard depends on.
///
...
pub(crate) fn require_terminal_event(
    ...
    Err(format!(
        "{provider_name} stream ended before the response reported an outcome (no \
         response.completed or response.incomplete). The connection was closed mid-generation, so \
         this turn's output is incomplete and is not being recorded as a result."
    ))
}
