...
- Web/PDF: `http(s)://...` URLs and local `.pdf` paths return markdown via the `bmd` CLI (raw capped markdown; no extraction prompt). Requires `bmd` installed and logged in.
