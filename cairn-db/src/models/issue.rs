...
pub struct Issue {
    ...
    /// Parent thread this issue was filed under. The two parent edges are
    /// mutually exclusive at creation, and this one carries only attention
    /// routing — a thread owns no branch, so a child of a thread derives its
    /// branches as if it had no parent.
    #[serde(default)]
