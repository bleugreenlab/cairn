...
pub(crate) const SETTINGS_CONTRACT: ResourceContract =
    ResourceContract {
        ...
        description: "The workspace-global settings document with every section: app prefs (maxThinkingTokens, mergeType, orphanCleanupDays, repoTargetSweepDays, bugReports, thinkingDisplayMode, memoryReviewEnabled, pendingMemoryThreshold, externalReplies), backends (enabledProviders, tierDefaults/tiers/backends plus a read-only model catalog and usage), git identities, provider accounts, keybinds, build services, and read-only GitHub status. patch routes each present key to its existing store; GitHub is read-only and OAuth account-add stays UI-only.",
        read_projections: NO_PROJECTIONS,
        related: NO_RELATED,
        cross_actions: NO_CROSS_ACTIONS,
        mutations: &[MutationSpec {
            ...
            optional: &[
                ...
                SETTINGS_MAX_THINKING_TOKENS,
                SETTINGS_ORPHAN_CLEANUP_DAYS,
                SETTINGS_REPO_TARGET_SWEEP_DAYS,
                SETTINGS_BUG_REPORTS,
                SETTINGS_THINKING_DISPLAY_MODE,
                SETTINGS_TRANSCRIPT_TEXT_SIZE,
                SETTINGS_TRANSCRIPT_DENSITY,
                SETTINGS_LOG_LEVEL,
                SETTINGS_LOG_RETENTION_DAYS,
                SETTINGS_MEMORY_TRIAGE_ENABLED,
                SETTINGS_MAX_OPEN_TRIAGE_ISSUES,
                SETTINGS_PENDING_MEMORY_THRESHOLD,
                SETTINGS_THREAD_COMPACT_THRESHOLD,
                SETTINGS_EXTERNAL_REPLIES,
                SETTINGS_SUBSCRIPTION_FEES,
                SETTINGS_OPENROUTER_ROUTING,
                SETTINGS_ROUTE_CALLS_VIA_OPENROUTER,
                SETTINGS_CHANNELS,
            ],
