...
pub(crate) const SETTINGS_CONTRACT: ResourceContract =
    ResourceContract {
        ...
        description: "The workspace-global settings document with every section: app prefs (maxThinkingTokens, mergeType, orphanCleanupDays, repoTargetSweepDays, bugReports, thinkingDisplayMode, memoryReviewEnabled, pendingMemoryThreshold, externalReplies), backends (tierDefaults/tiers/backends plus a read-only model catalog and usage), git identities, provider accounts, keybinds, build services, and read-only GitHub status. patch routes each present key to its existing store; GitHub is read-only and OAuth account-add stays UI-only.",
        read_projections: NO_PROJECTIONS,
        related: NO_RELATED,
        cross_actions: NO_CROSS_ACTIONS,
        mutations: &[MutationSpec {
