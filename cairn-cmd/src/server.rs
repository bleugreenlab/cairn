...
impl ServerHandler for CairnCmd {
    fn get_info(&self) -> ServerInfo {
        let mut instructions = "Cairn MCP server for agent orchestration.\n\n\
             ...
             - run: Execute an ordered batch of synchronous shell commands, inline code (`{code, interpreter}` — typescript/javascript via bun with zero-config `@cairn/sdk`, or stdlib python), and skill-script targets (parallel by default; `sequential` for ordered). If a successful worktree-bound run dirties the tree, pass `commit_msg` to commit it (`^` amends). Without a commit_msg, a run that dirties the worktree is restored to HEAD.\n\n\
